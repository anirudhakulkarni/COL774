...
def naive_bayes(X,Y,Y_train_raw,vocabs,super_vocab):
    ...
    for class_label in range(total_classes):
        ...
        for word in vocabs[class_label]:
    ...
    for class_label in range(5):
        ...
        for word in x:
            try:
                ...
                numer+=math.log(1)-math.log(super_dict_len)
...
if __name__ == '__main__':
    ...
    if part_num=='a':
        ...
        if test_prev_q1==1:
            ...
            np.save('X_test_clean_q1.npy',X_test_clean)
        print("Starting predictions")
        Pred_test=get_predictions(X_test_clean,psi_i,psi_y)
        print(np.count_nonzero(np.where(Pred_test==Y_test_raw))/len(Pred_test))


        # confusion matrix
        conf_mat=confusion_matrix(Y_test_raw,Pred_test)
        draw_confusion(conf_mat,"nb")
        ...
        Pred_test=get_predictions(X_test_clean_tr,psi_i,psi_y)
        print(np.count_nonzero(np.where(Pred_test==Y_test_raw))/float(len(Pred_test)))
        print(Pred_test)
        print(Pred_test.shape)

        # get predictions on training data
        Pred_train=get_predictions(X_train_clean_tr,psi_i,psi_y)
        print(Pred_train)
        print(Pred_train.shape)
        print(np.count_nonzero(np.where(Pred_train==Y_train_raw))/float(len(Pred_train)))
    elif part_num=='e':
        # read training data
        raw_data = pd.read_json(train_path,lines=True)
        Y_train_raw=np.array(raw_data['overall'])
        ...
        # confusion matrix
        conf_mat=confusion_matrix(Y_test_raw,Pred_test)
        draw_confusion(conf_mat,"nb_bi_test")

        conf_mat=confusion_matrix(Y_train_raw,Pred_train)
        draw_confusion(conf_mat,"nb_bi_train")
        ...
        # confusion matrix
        conf_mat=confusion_matrix(Y_test_raw,Pred_test)
        draw_confusion(conf_mat,"nb_L_test")

        conf_mat=confusion_matrix(Y_train_raw,Pred_train)
        draw_confusion(conf_mat,"nb_L_train")
    elif part_num=='g':
        # read training data
        raw_data = pd.read_json(train_path,lines=True)
        Y_train_raw=np.array(raw_data['overall'])
        ...
        # get predictions on training data
        Pred_train=get_predictions(X_train_clean,psi_i,psi_y)
        print(np.count_nonzero(np.where(Pred_train==Y_train_raw))/len(Pred_train))

        conf_mat=confusion_matrix(Y_test_raw,Pred_test)
        draw_confusion(conf_mat,"sum_test")
        conf_mat=confusion_matrix(Y_train_raw,Pred_train)
        draw_confusion(conf_mat,"sum_train")
